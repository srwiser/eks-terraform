...
resource "aws_iam_instance_profile" "node" {
  name = "eks-node-instance-profile-${var.cluster-name}"
  role = "${aws_iam_role.node.name}"
}
...
resource "aws_security_group_rule" "bastion-to-node" {
  description              = "Allow connection from public bastion to EKS nodes"
  from_port                = 22
  to_port                  = 22
  protocol                 = "tcp"
  cidr_blocks              = ["${var.bastion-cidr}"]
  security_group_id        = "${aws_security_group.node.id}"
  type                     = "ingress"
}

resource "aws_security_group_rule" "elb-to-node" {
  description              = "Allow traffic from Public elb to nodes"
  from_port                = 0
  protocol                 = "tcp"
  security_group_id        = "${aws_security_group.node.id}"
  source_security_group_id = "${var.elb-sg}"
  to_port                  = 65535
  type                     = "ingress"
}
...
resource "aws_autoscaling_group" "eks" {
  ...
  vpc_zone_identifier  = ["${var.eks-subnet-0}","${var.eks-subnet-1}"]
